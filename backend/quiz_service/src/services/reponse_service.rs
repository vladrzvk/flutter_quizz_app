...
impl ReponseService {
    ...
    pub async fn update(
        ...
        let existing = ReponseRepository::find_by_id(pool, id)
            .await?
            .ok_or_else(|| AppError::NotFound(format!("Reponse with id {} not found", id)))?;
    ...
    pub async fn create_bulk(
        pool: &PgPool,
        question_id: Uuid,
        requests: Vec<CreateReponseRequest>,
        ...
        if requests.is_empty() {
            return Err(AppError::BadRequest("Au moins une réponse doit être fournie".to_string()));
        }

        if requests.len() > 6 {
            return Err(AppError::BadRequest("Maximum 6 réponses par question".to_string()));
        }

-- ============================================
-- QUIZ SERVICE DATABASE - SCHEMA EXISTANT
-- ============================================
...
CREATE TABLE quizzes (
    ...
    -- NOUVEAU - Access Control (Phase 4)
    access_level VARCHAR(50) DEFAULT 'free' CHECK (access_level IN ('free', 'premium', 'freemium')),
...
-- ============================================
-- TABLE: sessions_quiz (MODIFIÉ - Phase 3)
-- ============================================
CREATE TABLE sessions_quiz (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    
    -- NOUVEAU - User tracking (Phase 3)
    user_id UUID,  -- NULL si guest/non-auth_service
...
-- Index sessions
CREATE INDEX idx_sessions_user ON sessions_quiz(user_id) WHERE user_id IS NOT NULL;
...
-- Questions (voir seed complet dans fichier séparé)
-- [Toutes les 10 questions du seed fourni]

DO $$
BEGIN
    RAISE NOTICE 'Quiz database initialized successfully!';
    RAISE NOTICE 'Tables created: 5 (domains, quizzes, questions, reponses, sessions_quiz, reponses_utilisateur)';
    RAISE NOTICE 'Domain geography seeded';
    RAISE NOTICE 'Sample quiz seeded with 10 questions';
END $$;
...
COMMENT ON COLUMN quizzes.access_level IS 'NOUVEAU Phase 4: free, premium, freemium';
COMMENT ON COLUMN quizzes.credit_cost IS 'NOUVEAU Phase 4: coût en crédits pour freemium';
...
COMMENT ON COLUMN sessions_quiz.user_id IS 'NOUVEAU Phase 3: NULL pour guest, UUID pour authentifié';
COMMENT ON COLUMN sessions_quiz.is_authenticated IS 'NOUVEAU Phase 3: TRUE si user authentifié';
